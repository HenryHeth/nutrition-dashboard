...
async function syncDay(date) {
  // `date` is the orchestrator's "anchor" — backfill a 14-day rolling window from it.
  // First-run safety: env override BACKFILL_DAYS lets us catch up larger gaps.
  const windowDays = parseInt(process.env.FATSECRET_BACKFILL_DAYS || '14', 10);
