...
async function syncDay(date) {
  ...
  for (let i = WINDOW_DAYS - 1; i >= 0; i--) {
    ...
    // Fitbit returns efficiency per sleep[] entry, not on summary.
    // Use the mainSleep entry's efficiency; fall back to minutes-weighted average.
    const sleeps = payload.sleep || [];
    let sleepScore = null;
    const main = sleeps.find(s => s.isMainSleep);
    if (main && main.efficiency != null) {
      sleepScore = main.efficiency;
    } else if (sleeps.length) {
      const total = sleeps.reduce((s, x) => s + (x.minutesAsleep || 0), 0);
      sleepScore = total > 0
        ? Math.round(sleeps.reduce((s, x) => s + (x.efficiency || 0) * (x.minutesAsleep || 0), 0) / total)
        : null;
    }
