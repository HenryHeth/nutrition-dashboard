...
async function syncDay(date) {
  ...
  for (const m of metrics) {
    ...
    const byType = {};
    for (const d of (m.details || [])) byType[d.type] = d.value;
    ...
    if ([sleep_hours, hrv, resting_hr, steps, body_battery, stress_score].some(v => v != null)) {
      await pool.query(
        `INSERT INTO daily_metrics
         (date, sleep_hours, hrv_ms, resting_hr, steps, body_battery, stress_score, source, raw, updated_at)
         VALUES ($1,$2,$3,$4,$5,$6,$7,'trainingpeaks',$8,NOW())
         ON CONFLICT (date) DO UPDATE SET
           ...
           updated_at   = NOW()`,
        [day, sleep_hours, hrv, resting_hr,
