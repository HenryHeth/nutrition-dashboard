...
async function syncDay(date) {
  ...
  for (const m of metrics) {
    ...
    // Index by type, also capture uploadClient per type for source attribution
    const byType = {};
    const clientByType = {};
    for (const d of (m.details || [])) {
      byType[d.type] = d.value;
      if (clientByType[d.type] == null) clientByType[d.type] = d.uploadClient || null;
    }

    // Stress / BodyBattery come back as arrays — pull the average-ish value (last element pattern)
    const arrAvg = v => Array.isArray(v) ? (v[v.length - 1] ?? null) : v;

    const sleep_hours  = byType[6]  ?? null;
    const hrv          = byType[60] ?? null;
    const resting_hr   = byType[5]  ?? null;
    const steps        = byType[58] ?? null;
    const body_battery = arrAvg(byType[64]);
    const stress_score = arrAvg(byType[62]);
    const sleep_source = sleep_hours != null ? labelClient(clientByType[6]) : null;
    ...
    const weight_kg    = byType[9]  ?? null;
    const waist_cm     = byType[27] ?? null;
    const chest_cm     = byType[26] ?? null;
    const shoulders_cm = byType[25] ?? null;
