...
async function syncDay(date) {
  ...
  for (const m of metrics) {
    ...
    // Some days have MULTIPLE entries per metric type from different uploadClients
    // (e.g. Garmin Health AND FitnessSyncer both report sleep). And Fitbit can
    // log 2-3 sleep records per day (main sleep + naps) — those should be summed.
    // Strategy: bucket by (type, client), sum within each bucket, then pick the
    // preferred client per metric (Fitbit > Garmin > anything else).
    //
    // For non-sleep numeric metrics we just take the first non-null value.
    const sumByTypeClient = {}; // type -> { client -> sum }
    const firstByType = {};      // type -> first non-null value (for non-summable metrics)
    for (const d of (m.details || [])) {
      ...
      sumByTypeClient[d.type] ||= {};
      sumByTypeClient[d.type][c] = (sumByTypeClient[d.type][c] || 0) + d.value;
      if (firstByType[d.type] === undefined) firstByType[d.type] = d.value;
    }

    // Per-metric source preference: Fitbit > Garmin > anything else
    const pickPreferred = type => {
      const buckets = sumByTypeClient[type];
      if (!buckets) return [null, null];
      if (buckets.fitbit != null) return [buckets.fitbit, 'fitbit'];
      if (buckets.garmin != null) return [buckets.garmin, 'garmin'];
    ...
    const [sleep_hours, sleep_source] = pickPreferred(6);
