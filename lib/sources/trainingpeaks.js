      ...
      await pool.query(
        `INSERT INTO daily_metrics
         ...
         ON CONFLICT (date) DO UPDATE SET
           -- Don't clobber Fitbit-direct sleep — only fill if no source set yet
           sleep_hours  = CASE WHEN daily_metrics.sleep_source IS NULL
                               THEN COALESCE(EXCLUDED.sleep_hours, daily_metrics.sleep_hours)
                               ELSE daily_metrics.sleep_hours END,
           sleep_source = COALESCE(daily_metrics.sleep_source, EXCLUDED.sleep_source),
