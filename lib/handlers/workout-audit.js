...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const data = rows.map(r => {
      ...
      // Flag = any exercise with sets missing weight OR reps.
      // For bodyweight weight gaps: forgot the override.
      // For weighted weight gaps: Garmin didn't capture the weight.
      // For rep gaps: Garmin didn't capture reps (hand-tracked or sensor missed).
      const flagged = missingWeightSets > 0 || missingRepsSets > 0;
      const flagReasons = [];
