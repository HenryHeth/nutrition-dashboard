...
module.exports = async (req, res) => {
  ...
  try {
    const [rows, missingRows, standards] = await Promise.all([
      ...
      // Include session_id (Garmin activityId) so we can deep-link.
      safeQuery(
        `SELECT ws.exercise, s.date, s.id AS session_id, COUNT(*) AS sets_missing
         FROM workout_sets ws
         JOIN workout_sessions s ON s.id = ws.session_id
         WHERE ws.exercise IS NOT NULL
           AND s.date >= CURRENT_DATE - $1::int
    ...
    const data = rows.map(r => {
      ...
      // Flag = any exercise with missing/zero weight sets. (High variance on
      // weighted exercises is normal progressive overload — not flagged.)
      // For bodyweight: forgot the override. For weighted: Garmin didn't capture.
      // Either way, missing data needs a fix in Garmin.
      const flagged = missingSets > 0;
      const flagReasons = [];
      if (flagged) {
        const where = isBodyweight ? '(bodyweight — forgot override)' : '(Garmin didn\'t capture)';
        flagReasons.push(`${missingSets} set${missingSets>1?'s':''} missing weight ${where} — fix in Garmin`);
      }
    ...
    const totalMissing = data.reduce((s, r) => s + r.missing_weight_sets, 0);
    const flaggedCount = data.filter(r => r.flagged).length;
    return res.status(200).json({
      success: true, days, count: data.length, data,
      summary: { total_missing_sets: totalMissing, flagged_exercises: flaggedCount }
    });
  } catch (e) {
