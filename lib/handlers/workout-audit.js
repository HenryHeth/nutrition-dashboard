...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const data = rows.map(r => {
      ...
      // Flag = bodyweight exercise with missing/zero weight sets. (High variance on
      // weighted exercises is normal progressive overload, not a problem.)
      const missingOnBodyweight = isBodyweight && missingSets > 0;
      const flagged = missingOnBodyweight;
      const flagReasons = [];
      if (missingOnBodyweight) flagReasons.push(`${missingSets} set${missingSets>1?'s':''} missing weight — fix in Garmin`);
