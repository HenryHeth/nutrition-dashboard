// /api/fitness/matt-synopsis?mode=bulk|cut|tri|balanced&criticality=gentle|balanced|blunt&days=7
// Pulls last N days of all sources, builds a context block, and asks Claude
// (in Matthew's voice) for a focused synopsis with both positives and concerns
// scaled to Paul's current training mode.
...
async function pullContext(days) {
  const rows = await Promise.all([
    safeQuery(
      `SELECT date, calories, protein_g, carbs_g, fat_g, fiber_g
       FROM daily_nutrition
       WHERE date >= CURRENT_DATE - $1::int AND calories > 0
       ORDER BY date`,
      [days]
    ),
    safeQuery(
      `SELECT date, sleep_hours, sleep_score, sleep_source, hrv_ms, resting_hr, steps, body_battery, stress_score
       FROM daily_metrics
    ...
    safeQuery(
      `SELECT date, ROUND(ctl::numeric, 1) ctl, ROUND(atl::numeric, 1) atl, ROUND(tsb::numeric, 1) tsb
       FROM fitness_load
       WHERE date >= CURRENT_DATE - $1::int
       ORDER BY date`,
      [days]
    ...
    safeQuery(
      `SELECT s.date,
       ...
       WHERE s.date >= CURRENT_DATE - $1::int AND ws.reps IS NOT NULL
       GROUP BY s.date
       ORDER BY s.date`,
      [days]
    ),
    ...
    safeQuery(
      `SELECT date, ROUND(weight_kg * 2.20462::numeric, 1) AS weight_lbs
       FROM weight
       WHERE date >= CURRENT_DATE - $1::int
       ORDER BY date`,
      [days]
...
module.exports = async (req, res) => {
  ...
  const days = Math.min(30, Math.max(3, parseInt(req.query.days) || 7));
  ...
  try {
    const ctx = await pullContext(days);
