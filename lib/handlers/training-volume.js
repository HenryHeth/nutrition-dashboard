...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Quick missing weight + missing reps counts for the dashboard's
    // proactive indicator. Same date range as the chart.
    const missingRow = await safeQuery(
      `SELECT
         COUNT(*) FILTER (WHERE ws.weight_lbs IS NULL OR ws.weight_lbs = 0)::int AS missing_weight,
         COUNT(*) FILTER (WHERE ws.reps IS NULL OR ws.reps = 0)::int             AS missing_reps
       FROM workout_sets ws
       JOIN workout_sessions s ON s.id = ws.session_id
    ...
    const missing_weight_sets = missingRow[0]?.missing_weight || 0;
    const missing_reps_sets   = missingRow[0]?.missing_reps   || 0;

    return res.status(200).json({
      success: true, count: data.length, data,
      missing_weight_sets, missing_reps_sets
    });
  } catch (e) {
